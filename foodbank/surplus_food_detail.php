<?php
   ...
   $title = "Surplus Food Details";
   $code = "surplus_food_detail";
   $status = "1";
               ...
               <div class="card">
                  <div class="row g-0">
                     <?php
                        ...
                        {
                        	$id = $_REQUEST["id"];
                            $result=mysqli_query($conn,"select * from tbl_surplus_meals as sm left join tbl_meal as m on m.meal_id=sm.meal_id left join tbl_restaurant as r on r.restaurant_id=sm.restaurant_id where sm.surplus_id='".$_REQUEST["id"]."'");
...
<script>
    // Function to open the surplus meal popup
    function openSurplusPopup(surplusId) {
        // You can use surplusId to fetch additional details if needed
        // For now, let's assume you want to open a simple popup for quantity input

        var quantity = prompt("Enter the quantity:");

        // Check if the user entered a quantity
        if (quantity !== null) {
            // Perform an AJAX request to insert data into tbl_foodbank_meal
            // You'll need to create a separate PHP file to handle this AJAX request
            // For simplicity, I'll use jQuery for the AJAX request

            $.ajax({
                type: "POST",
                url: "insert_foodbank_meal.php",  // Replace with your PHP file handling the insertion
                data: {
